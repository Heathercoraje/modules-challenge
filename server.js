...
const handler = (req, res) => {
  ...
  if (url === '/') {
    ...
    fs.readFile(path.join(__dirname, '/public/stylesheet.css'), 'utf8', (err, file) => {
      /* istanbul ignore if */
      if (err) {
        res.writeHead(500, {'content-type': 'text/plain'});
        res.end('server error');
    ...
    fs.readFile(path.join(__dirname, 'public/js/request.js'), 'utf8', (err, file) => {
      /* istanbul ignore if */
      if (err) {
        res.writeHead(500, {'content-type': 'text/plain'});
        res.end('server error');
    ...
    fs.readFile(path.join(__dirname, 'public/js/index.js'), 'utf8', (err, file) => {
      /* istanbul ignore if */
      if (err) {
        res.writeHead(500, {'content-type': 'text/plain'});
        res.end('server error');
